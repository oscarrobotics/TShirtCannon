...
public class cannonControl extends Command {
    ...
    public cannonControl() {
        requires(Robot.cannonControlSub);
    }
    public boolean isDone;

    // Called just before this Command runs the first time
    protected void initialize() {
        Robot.i2cLEDs.setLEDs((byte) 74);
    }

    // Called repeatedly when this Command is scheduled to run
    protected void execute() {
        isDone = false;
        if (Robot.oi.controlStick.getRawButton(2)) { // If we start with safety pressed, check trigger
            RobotMap.cannonLight.set(Relay.Value.kForward);
            if (Robot.oi.controlStick.getRawButton(1)) { // If trigger is pressed, fire
